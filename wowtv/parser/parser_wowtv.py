...
def parser(data):
    ...
    n_rn = ns1.find('div', attrs={'class': 'section-content'})
    ## 본문이 다른 태그 아래 있는 경우
    n_body = ns1.find('div', attrs={'class': 'box-news-body'})
    ## 이미지 글 제거
    try:
        photos = n_rn.find_all('img')
        for photo in range(len(photos)):
            n_rn.find('img').decompose()
    except:
        pass
    ...
    for i in range(len(content_list)):
        ...
        if type(content) is Comment:
            ...
            ## media_summary 첫번째
            if '<div class="box-headline">' in str(content):
    ...
    try : 
        ...
        LOGGER.info(f'Wowtv_parser {id} End')
...
if __name__ == '__main__':
    ...
    loguru.logger.add(
        sink=f'./logs/wowtv_parser_{times}.log',
        format=log_format,
        enqueue=True,
        level='INFO'.upper(),
        rotation='10 MB',
    ...
    LOGGER.info("Wowtv parser started")
