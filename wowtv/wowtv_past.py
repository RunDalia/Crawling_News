...
class PastWowtv:
    ...
    def zmq_connect(self):
        ...
        perser.connect("tcp://127.0.0.1:5215")
        return perser
    ...
    def url_list(self):
        ...
        for i in range(len(cate_values)):
            ...
            try:
                res01 = requests.get(wowtv_url, headers=self.header)
                soup = BeautifulSoup(res01.content, 'lxml')
                temp = soup.find('div', attrs={'class': 'box-paginate'})
                last_index = int(temp.find('a', attrs={'class': 'last'}).get('href').split('=')[-1])
                self.LOGGER.info(f'{cate_key[i]} last index number is : {last_index}')
                for idx in range(0, last_index, 20):
                    idx_url = wowtv_url + f'&currentIndex={idx}'
                    res02 = requests.get(idx_url, headers=self.header)
    ...
    def insert_mongo(self, all_news, cate_key, cate_values, sub_values):
        ...
        for news in all_news:
            ...
            ## 중복 처리
            ### 과거 기사 수집은 중복처리 삭제 요망
            try:
                # print(mongo_data)
                self.db_connetion.mongodb_connection(mongo_data, self.conn)
