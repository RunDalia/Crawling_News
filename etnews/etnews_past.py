...
class PastETNews:
    ...
    def url_list(self):
        ...
        for i in range(len(cate_values)):
            ...
            for k in range(len(sub_key)):
                ...
                page = 300
                for j in range(page):
                    all_url = f'https://www.etnews.com/news/section.html?id1={cate_values[i]}&id2={subvalue_list}&page={j}'
                    try:
                        res = requests.get(all_url, headers=self.header)
    ...
    def crawler(self, news, cate_key, cate_values, subkey_list, subvalue_list) -> dict:
        ...
        etnews_list['news_date'] = dt.datetime.strptime(etnews_list['article_date'], '%Y.%m.%d %H:%M').strftime(
            '%Y%m%d')
